...
class RandomWaypoint(object):
    ...
    def __init__(self, nr_nodes, dimensions, height, velocity=(0.1, 1.), wt_max=None):
        '''
    ...
    def __iter__(self):
        ...
        while True:
            ...
            z = np.zeros(self.nr_nodes) * self.height
            yield np.dstack((x,y,z,velocity))[0]

class StochasticWalk(object):
    
    def __init__(self, nr_nodes, dimensions, height, FL_DISTR, VELOCITY_DISTR, WT_DISTR=None, border_policy='reflect'):
        '''
    ...
    def __iter__(self):
        ...
        while True:
            ...
            yield np.dstack((xy[:,0],xy[:,1],z,velocity))[0]

class RandomWalk(StochasticWalk):
    
    def __init__(self, nr_nodes, dimensions, height=1.5, velocity=1., distance=1., border_policy='reflect'):
        '''
        ...
        StochasticWalk.__init__(self, nr_nodes, dimensions, height, FL_DISTR, VELOCITY_DISTR,border_policy=border_policy)


class RandomDirection(StochasticWalk):

    def __init__(self, nr_nodes, dimensions, height=1.5, wt_max=None, velocity=(0.1, 1.), seed=10,
...
class TruncatedLevyWalk(StochasticWalk):
    ...
    def __init__(self, nr_nodes, dimensions, height=1.5, velocity=1., seed=10, FL_EXP=-20.6, FL_MAX=100., WT_EXP=-1.8, WT_MAX=10., border_policy='reflect'):
        ...
        if WT_EXP and WT_MAX:
            ...
            WT_DISTR = None
        VELOCITY_DISTR = lambda FD: np.sqrt(FD)*velocity
...
def drone_mobility(nr_nodes, dimensions, velocity_mean=1., alpha=1., variance=1., z_low_limit=20):
    '''
    Required arguments:
      ...
      *dimensions*:
        Tuple of Integers, the x, y, and z dimensions of the simulation area.
    ...
    keyword arguments:
      ...
      *z_low_limit*:
        The lowest height the Drone can reach
    '''
    ...
    x = U(0, MAX_X, NODES)
    y = U(0, MAX_Y, NODES)
    z = U(LOW_Z, MAX_Z, NODES)
    velocitymean = velocity_mean
    angle_mean = np.random.uniform(0,2*np.pi,size=nr_nodes)
    pitch_mean = np.zeros(nr_nodes)+0.0


    velocity = np.zeros(nr_nodes)+velocitymean
    theta = np.random.uniform(0,2*np.pi,size=nr_nodes)
    pitch = np.zeros(nr_nodes)+0.0
    ...
    while True:
        ...
        b = np.where(y>MAX_Y)[0]
        y[b] = 2*MAX_Y - y[b]; theta[b] = -theta[b]; angle_mean[b] = -angle_mean[b]
        b = np.where(z<LOW_Z)[0]
        z[b] = 2*LOW_Z - z[b]; pitch[b] = np.pi-pitch[b]; pitch_mean[b] = np.pi-pitch_mean[b]
        b = np.where(z>MAX_Z)[0]
        z[b] = 2*MAX_Z - z[b]; pitch[b] = np.pi-pitch[b]; pitch_mean[b] = np.pi-pitch_mean[b]

        velocity = (alpha * velocity +
                alpha2 * velocitymean +
                alpha3 * np.random.uniform(0,1,size=nr_nodes))
        ...
        pitch = (alpha * pitch +
                    alpha2 * pitch_mean +
                    alpha3 * np.random.uniform(0,0.2,size=nr_nodes))


        yield np.dstack((x,y,z,velocity))[0]

        
def reference_point_group(nr_nodes, dimensions, height=1.5, velocity=(0.1, 1.), seed=10, aggregation=0.2):
    '''
    ...
    x = U(0, MAX_X, NODES)
    y = U(0, MAX_Y, NODES)
    z = np.zeros(sum(nr_nodes)) + height
    #FIXME: The following line was added by incompetent gorilla
    velocity = np.ones(sum(nr_nodes))
    theta = U(0, 2*np.pi, NODES)
    costheta = np.cos(theta)
    sintheta = np.sin(theta)
