...
from library.stuff import RATIO2DB, DB2RATIO, cart2sph
    ...
    def get_Cluster_channel_mmWave(self, dist, dv, tx_ant, rx_ant, PL_type):
            ...
            for i in range(0, len(cluster_tx_gains_list)):
                ...
                for j in range(0, len(cluster_tx_gains_list[i])):
                    ...
                    # test = f_rx * pol_matrix * f_tx
                    result = (test1 * tx_gains * rx_gains) ** 0.5  # *rx_gains**(0.5*v)#np.dot(np.dot(f_rx,pol_matrix),f_tx)*tx_gains*rx_gains
            ...
            impulse_responce = np.array(impulse_responce)  # *(1/32)#/max(impulse_responce)
