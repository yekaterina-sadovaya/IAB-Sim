...
class STAT_container_class:
    ...
    which is collected and updated
    throughout the simulation process.
    Some of these parameters are used for
    calculating the final results while
    others might be needed for intermediate
    statistics
    """
    ...
    def __init__(self):
        ...
        # in this data, DL packets correspond to DgNB buffer; UL packets to UEs;
        # buffers of IAB nodes are realized separately
        self.packet_traffic = {key: {'BH': {'DL': {k: [] for k in range(gl.n_UEs)},
                                            'UL': {k: [] for k in range(gl.n_UEs)}},
                                     'AC': {'DL': {k: [] for k in range(gl.n_UEs)},
                                            'UL': {k: [] for k in range(gl.n_UEs)}}}
                               for key in keys}
        ...
        self.BH_counter = {1: np.zeros(3), 2: np.zeros(3),
                           3: np.zeros(3), 4: np.zeros(3)}     # counter required to satisfy optimal allocations
